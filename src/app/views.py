...
class SentSpotifyLinkListView(BaseAppView, SingleTableMixin, filter_views.FilterView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.select_related(
            "link", "link__artist", "link__album", "link__track", "chat", "sent_by"
        ).filter(
            chat__in=TelegramUser.get_chats(self.request.user.profile.telegram_user_id)
        )
...
class SavedSpotifyLinkListView(BaseAppView, SingleTableMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            self.request.user.profile.telegram_user.saved_links.select_related(
                "link", "link__artist", "link__album", "link__track", "user"
            )
            .prefetch_related("link__album__genres")
            .all()
        )
...
class FollowedArtistListView(BaseAppView, SingleTableMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            self.request.user.profile.telegram_user.followed_artists.select_related(
                "artist"
            ).all()
        )
...
class FollowedArtistDetailView(BaseAppView, DetailView):
    ...
    def get_queryset(self):
        return self.request.user.profile.telegram_user.followed_artists.select_related(
            "artist"
        ).all()
