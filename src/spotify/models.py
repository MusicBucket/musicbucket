...
class BaseSpotifyModel(models.Model):
    spotify_id = models.CharField(
        verbose_name=_("Spotify ID"), max_length=250, unique=True
    )
    name = models.CharField(verbose_name=_("Name"), max_length=250)
    href = models.URLField(
        verbose_name=_("Spotify API Href"), max_length=250, blank=True
    )
    ...
    popularity = models.IntegerField(
        verbose_name=_("Popularity"), blank=True, null=True
    )
...
class Artist(EmojiMixin, BaseSpotifyModel):
    ...
    genres = models.ManyToManyField(
        Genre, verbose_name=_("Genres"), related_name="artists"
    )
...
class Album(EmojiMixin, BaseSpotifyModel):
    ...
    release_date_precision = models.CharField(
        verbose_name=_("Release date precision"),
        choices=RELEASE_DATE_PRECISIONS,
        max_length=250,
    )
    genres = models.ManyToManyField(
        Genre, verbose_name=_("Genres"), related_name="albums"
    )
    artists = models.ManyToManyField(
        Artist, verbose_name=_("Artists"), related_name="albums"
    )
    ...
    def get_or_create_from_spotify_album(cls, spotify_album: {}):
        ...
        try:
            ...
            with transaction.atomic():
                image_url = (
                    spotify_album["images"][0]["url"] if spotify_album["images"] else ""
                )
                album = cls.objects.create(
                    ...
                    release_date=cls.parse_release_date(
                        spotify_album.get("release_date"),
                        spotify_album.get("release_date_precision"),
                    ),
    ...
    def parse_release_date(
        cls, release_date: str, release_date_precision
    ) -> datetime.date:
...
class Track(EmojiMixin, BaseSpotifyModel):
    ...
    number = models.PositiveIntegerField(
        verbose_name=_("Track number"), blank=True, null=True
    )
    duration = models.PositiveIntegerField(
        verbose_name=_("Duration (in ms)"), blank=True, null=True
    )
    explicit = models.BooleanField(verbose_name=_("Explicit"), blank=True, null=True)
    preview_url = models.URLField(
        verbose_name=_("Preview URL"), blank=True, max_length=250
    )
    album = models.ForeignKey(
        Album, verbose_name=_("Album"), related_name="tracks", on_delete=models.CASCADE
    )
    artists = models.ManyToManyField(
        Artist, verbose_name=_("Artists"), related_name="tracks"
    )
...
class FollowedArtist(models.Model):
    ...
    followed_at = models.DateTimeField(
        verbose_name=_("Followed at"), default=timezone.now
    )
    last_lookup = models.DateTimeField(
        verbose_name=_("Last lookup"), blank=True, null=True
    )
...
class PlayedTracksInfo(models.Model):
    ...
    played_tracks = models.ManyToManyField(
        Track, verbose_name=_("Played tracks"), through="PlayedTrack"
    )
...
class PlayedTrack(models.Model):
    played_tracks_info = models.ForeignKey(
        PlayedTracksInfo, verbose_name=_("User"), on_delete=models.CASCADE
    )
    track = models.ForeignKey(Track, verbose_name=_("Track"), on_delete=models.PROTECT)
    played_at = models.DateTimeField(verbose_name=_("Played at"))  # Timezone Aware
    played_at_ms = models.BigIntegerField(
        verbose_name=_("Played at ms")
    )  # Timezone NOT Aware
