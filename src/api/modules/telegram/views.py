...
class TelegramUserListCreateAPIView(
    rf_mixins.UpdateModelMixin, generics.ListCreateAPIView
):
...
class TelegramChatListCreateAPIView(
    rf_mixins.UpdateModelMixin, generics.ListCreateAPIView
):
    """This view allows also to update a registry if it already exists"""
...
class StatsAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        telegram_chat = get_object_or_404(
            TelegramChat, telegram_id=self.kwargs.get("chat__telegram_id")
        )
    ...
    def _get_users_with_chat_link_count(chat: TelegramChat):
        users_with_chat_link_count = (
            TelegramUser.objects.filter(sent_links__chat=chat)
            .annotate(sent_links_chat__count=Count("sent_links"))
            .order_by("-sent_links_chat__count")
        )
        return users_with_chat_link_count

    @classmethod
    def _get_most_sent_genres(cls, chat: TelegramChat) -> {}:
        links = SpotifyLink.objects.prefetch_related("sent_links").filter(
            sent_links__chat=chat
        )
        genres = []
        for link in links:
            if link.genres:
