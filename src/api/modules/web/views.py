...
class StatsAPIView(generics.RetrieveAPIView):
    ...
    def _get_most_sent_artists():
        most_sent_artists = Artist.objects.all().annotate(sent_count=Count('links__sent_links')) \
                                .order_by('-sent_count').values()[:10]
        most_sent_artists = [{
    ...
    def _get_last_sent_albums():
        last_sent_albums = Album.objects.filter(links__link_type=SpotifyLink.TYPE_ALBUM) \
                               .order_by('-links__sent_links__sent_at').distinct()[:10]
