...
class Command(BaseCommand):
    ...
    def _migrate_telegram_users(self):
        ...
        for bot_user in bot_models.User.objects.all():
            self.stdout.write(
                f"Migrating user {bot_user.username or bot_user.first_name}"
            )
            ...
            lastfm_user, was_created = LastfmUser.objects.get_or_create(
                ...
                defaults={
                    "user": TelegramUser.objects.get(
                        telegram_id=bot_lastfm_user.user.id
                    )
                },
            )
            if not was_created:
    ...
    def _migrate_tracks(self):
        ...
        for bot_track in bot_models.Track.objects.all():
            ...
            try:
                ...
                self.stdout.write(
                    f"WARNING: ALBUM NOT FOUND with Spotify ID {bot_track.album_id}"
                )
                continue
        ...
        for bot_track_artist in bot_models.TrackArtistThrough.objects.all():
            ...
            try:
                ...
                track.artists.add(
                    Artist.objects.get(spotify_id=bot_track_artist.artist.id)
                )
    ...
    def _migrate_links(self):
        ...
        for bot_link in bot_models.Link.objects.all():
            self.stdout.write(
                f"Migrating link {bot_link.url} of type {bot_link.link_type}"
            )
            link_type = ""
    ...
    def _clean_orphan_links(self):
        ...
        for link in SpotifyLink.objects.all():
            ...
            if link.link_type == SpotifyLink.TYPE_ARTIST and not link.artist:
                self.stdout.write(
                    f"Found orphan link: {link.url} ({link.link_type}). Deleting."
                )
                link.delete()
                ...
                link.delete()
        self.stdout.write(
            f"---------- End cleaning orphan links. Deleted {orphan_links_count} links"
        )
